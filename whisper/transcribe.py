    ...
    **decode_options,
    ...
    with tqdm.tqdm(total=num_frames, unit='frames', disable=verbose is not False) as pbar:
        while seek < num_frames:
            ...
            uncasted_segment = pad_or_trim(
                mel[:, seek:], N_FRAMES).to(model.device)
            ...
            ideal_number_of_tokens = int(
                duration * SAMPLE_RATE // samples_per_token)
            ...
            with torch.no_grad():
                logits = model(uncasted_segment.unsqueeze(0),
                               tokens_ts.unsqueeze(0))

            # layers * heads * tokens * frames
            weights = torch.concatenate(QKs)
            ...
            jumps = np.pad(np.diff(alignment.index1s), (1, 0),
            ...
            all_segments[-1]['word_timestamps'] = [
                dict(word=word, begin=timestamp_offset +
                     begin, end=timestamp_offset + end)
